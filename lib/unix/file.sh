...
.file.make_executable() {
  ...
  if [[ -f "${file}" && -n $(head -1 $1 | ${_bashmatic_grep} '#!.*(bash|ruby|env)') ]]; then
...
file.gsub() {
  ...
  ${_bashmatic_grep} -q "${find}" "${file}" || return 0
...
file.size() {
  _bashmatic_os=${_bashmatic_os:-$(uname -s)}
  if [[ "Linux" == ${_bashmatic_os} ]]; then
    stat -c %s "$1"
  else
    file.stat "$1" st_size
  fi
