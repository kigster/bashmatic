...
export _bashmatic_db_config=${_bashmatic_db_config:-"${HOME}/.db/database.yml"}
declare -a _bashmatic_db_connection

unset _bashmatic_db_username
unset _bashmatic_db_password
unset _bashmatic_db_host
unset _bashmatic_db_database
...
db.config.init() {
  export _bashmatic_db_connection=(host database username password)
}
...
db.config.parse() {
  local db="$1"
  [[ -z ${db} ]] && return 1
  [[ -f ${_bashmatic_db_config} ]] || return 2
  db.config.init
  local -a script=("require 'yaml'; h = YAML.load(STDIN); ")
  for field in "${_bashmatic_db_connection[@]}"; do
    script+=("h.key?('${db}') && h['${db}'].key?('${field}') ? print(h['${db}']['${field}']) : print('null'); print ' '; ")
  done
  cat "${_bashmatic_db_config}" | ruby -e "${script[*]}"
}

db.config.connections() {
  [[ -f ${_bashmatic_db_config} ]] || return 2
  cat "${_bashmatic_db_config}" | ruby -e "require 'yaml'; h = YAML.load(STDIN); puts h.keys.join(\"\n\")"
}

db.config.set-file() {
  [[ -s "$1" ]] || return 1
  export _bashmatic_db_config="$1"
}

db.config.get-file() {
  echo "${_bashmatic_db_config}"
}
...
db.psql.connect() {
  ...
  if [[ -z ${dbname} ]]; then
    h1 "USAGE: db.connect connection-name" \
      "WHERE: connection-name is defined by your ${bldylw}${_bashmatic_db_config}${clr} file."
    return 0
  fi

  local tempfile=$(mktemp /tmp/.bashmatic.db.${RANDOM} || exit 1)
  db.psql.args.config "${dbname}" >"${tempfile}" || return 1
...
db.psql.args() {
  if [[ -z "${_bashmatic_db_database}" || -z "${_bashmatic_db_host}" ]]; then
    if [[ -n "$1" ]]; then
    ...
    export PGPASSWORD="${_bashmatic_db_password}"
    printf -- "-U ${_bashmatic_db_username} -h ${_bashmatic_db_host} ${_bashmatic_db_database}"
  fi
}
