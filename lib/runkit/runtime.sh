...
export LibRun__Inspect__SkipFalseOrBlank=0

export LibRun__AbortOnError__Default=0
export LibRun__ShowCommandOutput__Default=0
export LibRun__AskOnError__Default=0
export LibRun__ShowCommand__Default=1
...
.run.bundle.exec.with-output() {
  export LibRun__ShowCommandOutput=1
  .run.bundle.exec "$@"
}
...
.run.eval() {
  ...
  if [[ ${LibRun__ShowCommandOutput} -eq 1 ]]; then
    printf "${clr}\n"
    eval "${command}"
    export LibRun__LastExitCode=$?
  else
...
run.print-command() {
  ...
  [[ ${LibRun__ShowCommandOutput} -eq 1 ]] && {
  ...
  if [[ "${LibRun__ShowCommand}" -eq 0 ]]; then
...
.run.exec() {
  ...
  if [[ -n ${BASHMATIC_DEBUG} && ${LibRun__Verbose} -eq 1 ]]; then
    run.inspect
  fi
  ...
  while [[ -n ${LibRun__LastExitCode} && ${LibRun__LastExitCode} -ne 0 ]] &&
    ...
    [[ ${tries} -gt 1 && ${__Previous__ShowCommandOutput} -eq 1 ]] &&
      export LibRun__ShowCommandOutput=0
  ...
  [[ ${LibRun__ShowCommandOutput} -eq 1 ]] && { echo; }

  if [[ ${LibRun__LastExitCode} -eq 0 ]]; then
    if [[ ${LibRun__ShowCommand} -eq 1 ]]; then
      command-spacer
    ...
    [[ ${LibRun__ShowCommandOutput} -eq 0 && -s ${run_stderr} ]] && {
      stderr_printed=true
      echo && stderr ${run_stderr}
    }

    if [[ ${LibRun__AskOnError} -eq 1 ]]; then
      run.ui.ask 'Ignore this error and continue?'

    elif [[ ${LibRun__AbortOnError} -eq 1 ]]; then
...
run.inspect.set-skip-false-or-blank() {
  ...
  [[ -z "${value}" ]] && export LibRun__Inspect__SkipFalseOrBlank=1
}
...
run.inspect-variable() {
  ...
  if [[ -n "${var_value}" ]]; then
    ...
    elif [[ "${var_value}" == "1" || "${var_value}" == "1" ]]; then
      value="${value_check}"
      color="${bldgrn}"
    elif [[ "${var_value}" == "0" || "${var_value}" == "0" ]]; then
      value="${value_off}"
      color="${bldred}"
    fi
  else
  ...
  if [[ ${LibRun__Inspect__SkipFalseOrBlank} -eq 1 && "${value}" == "${value_off}" ]]; then
    return 0
  fi
