...
export _bashmatic__colorloaded=${_bashmatic__colorloaded:-"0"}
...
color.enable() {
  if ((_bashmatic__colorloaded)); then
    ...
    export _bashmatic__colorloaded=1
  fi
}
...
color.disable() {
  ...
  export _bashmatic__colorloaded=0

  #trap reset-color EXIT
}

((_bashmatic__colorloaded)) || color.enable
