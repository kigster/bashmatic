...
export _bashmatic_spacer_width="${_bashmatic_spacer_width:-4}"
...
.output.current-screen-width() {
  ...
  util.is-a-function "${func}" || {
    echo "invalid strategy: ${strategy}, function ${func} does not exist." >&2
    return 1
  }
  ${func}
}
...
.output.screen-height() {
  if [[ ${_bashmatic_os:-$(uname -s)} == 'Darwin' ]]; then
    h=$(stty -a 2>/dev/null | grep rows | awk '{print $4}')
  elif [[ ${_bashmatic_os} == 'Linux' ]]; then
...
.output.set-indent() {
  ...
  [[ ${shift} -ge 0 && ${shift} -lt 100 ]] && {
    export _bashmatic_spacer_width=${shift}
  }
}
...
dbg() {
  is-dbg && printf "${txtgrn}[ debug | ${txtylw}$(time.now.with-ms)${txtgrn} ] ${txtblu}$(txt-info)$*${clr}\n" >&2
  resturn 0
}
