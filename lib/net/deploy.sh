...
# Set me to the ${_bashmatic_grep}  regular expression to filter `netstat -rn` with.
...
.deploy.check-vpn() {
  netstat -rn | ${_bashmatic_grep} "${LibDeploy__VpnSubnet}" >/dev/null
}
...
.deploy.vpn-error() {
  ...
  if [[ -n "$(netstat -rn | grep utun)" ]]; then
    ...
    printf "${bldylw}"
    netstat -rn | ${_bashmatic_grep} '^[0-9].*tun'
    printf "${clr}\n"
  fi
...
deploy.slack() {
  ...
  [[ ${LibRun__DryRun} -eq 1 ]] && run "send to slack [${text}]"
}
