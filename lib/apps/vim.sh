...
vim.gvim-off() {
  ...
  # append to ~/.bashrc
  ${_bashmatic_grep} -q "${regex_from}" ${LibVim__initFile} || echo "${regex_to}" >>${LibVim__initFile}
  ${_bashmatic_grep} -q "^gvim\.o" ${LibVim__initFile} || echo "gvim.off" >>${LibVim__initFile}
...
vim.gvim-on() {
  ...
  # append to ~/.bashrc
  ${_bashmatic_grep} -q "${regex_from}" ${LibVim__initFile} || echo "${regex_to}" >>${LibVim__initFile}
  ${_bashmatic_grep} -q "^gvim\.o.*" ${LibVim__initFile} || echo "gvim.on" >>${LibVim__initFile}
