...
declare -a _bashmatic__iserrors
declare -a _bashmatic__isvalues
export _bashmatic__issessionname
...
function validations.begin() {
  export _bashmatic__iserrors=()
  export _bashmatic__isvalues=()
  export _bashmatic__issessionname="$1"
}
...
function validations.add-error() {
  ...
  is.not-blank "${error}" && is.not-blank "${value}" && {
    export _bashmatic__iserrors+=("${error}")
    export _bashmatic__isvalues+=("${value}")
  }
}

function validations.print-errors() {
  local error_count="${#_bashmatic__iserrors}"
  ((error_count)) || return 0

  is.not-blank "${_bashmatic__issessionname}" &&
    hl.salmon "Reporing errors for ${_bashmatic__issessionname}"

  for i in $(seq 1 "${error_count}"); do
    local error=${_bashmatic__iserrors[$((i - 1))]}
    local value=${_bashmatic__isvalues[$((i - 1))]}
