...
util.arch() {
  echo -n "${_bashmatic_os}-$(uname -m)-$(uname -p)" | tr 'A-Z' 'a-z'
}
...
util.whats-installed() {
  declare -a hb_aliases=($(alias | ${_bashmatic_grep} 'hb\..*=' | sedx 's/alias//g; s/=.*$//g'))
...
util.functions-starting-with() {
  ...
  set | ${_bashmatic_grep} '^[^ ].* \(\) $' | ${_bashmatic_grep} "^${prefix}" | sedx 's/[\(\)]//g;' | ${extra_command} | tr '\n ' ' '
}

util.functions-starting-with-lines() {
  local prefix="${1}"
  local extra_command=${2:-"cat"}
  set | ${_bashmatic_grep} '^[^ ].* \(\) $' | ${_bashmatic_grep} "^${prefix}" | sedx 's/[\(\)]//g;' | ${extra_command}
}
...
util.functions-matching() {
  local prefix="${1}"
  local extra_command=${2:-"cat"}
  set | ${_bashmatic_grep} "^${prefix}" | sedx 's/[\(\)]//g;' | tr -d ' ' | tr '\n' ' '
}
...
export _bashmatic_util_sed_command=
...
#———————————————————————————————————————————————————————
sedx.cache-command() {
  if [[ -z "${_bashmatic_util_sed_command}" ]]; then
    local sed_path
    local sed_command
    local os
    ...
    sed_command="${_bashmatic_util_sed_command}"
  fi

  sed_command="${sed_command} -r -e "
  printf "%s" "${sed_command}"

  [[ -z ${_bashmatic_util_sed_command} ]] &&
    export _bashmatic_util_sed_command="${sed_command}"
}

sedx() {
  [[ -z ${_bashmatic_util_sed_command} ]] &&
    export _bashmatic_util_sed_command="$(sedx.cache-command)"

  ${_bashmatic_util_sed_command} "$*"
}
