...
interrupted() {
  export _bashmatic__interrupted=true
}
...
ruby.gemfile-lock-version() {
  ...
  ${_bashmatic_grep} " ${gem} \([0-9]" Gemfile.lock | sed -e 's/[\(\)]//g' | awk '{print $2}'
}
...
ruby.stop() {
  ...
  local procs=$(ps -ef | ${_bashmatic_grep} "${regex}" | ${_bashmatic_grep} -v grep | awk '{print $2}' | sort | uniq | wc -l)
  [[ ${procs} -eq 0 ]] && {
    info: "No ruby processes were found."
    return 0
  }

  local -a pids=$(ps -ef | ${_bashmatic_grep} "${regex}" | ${_bashmatic_grep} -v grep | awk '{print $2}' | sort | uniq | tr '\n' ' -p ')
  ...
  ps -ef | ${_bashmatic_grep} "${regex}" | ${_bashmatic_grep} -v grep | awk '{print $2}' | sort | uniq | xargs kill -9
}
