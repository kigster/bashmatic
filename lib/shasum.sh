...
if bashmatic.bash.version-four-or-later; then
  ...
  # @note Call `dbg-on` to enable additional debugging info
...
# @example shasum.all-files *.cpp
# @note Only computes SHA of the files provided, does not recurse into folders
shasum.all-files() {
