[separator=::]
= Bashmatic® v2.3.1:: BASH-based DSL helpers for humans, sysadmins, and fun.
:doctype: book
...
image::doc/img/bashmatic-install.png[Installer,width=70%,align=center]
...
Then you will have insta-access to all of the convenience functions Bashmtic offers (all 700+ of them).

And while loading the `init.sh` file adds time to your login initialzation, and may auto-update the Bashmatic library, there may be good reasons when you prefer NOT to automatically load it on login.
