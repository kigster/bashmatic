...
bash -c "$(curl -fsSL https://bashmatic.re1.re); \
    bashmatic-install -d -v -f -b v3.5.1 -H ~/workspace/bashmatic"
----
...
function install_bashmatic() {
  ...
  [[ -d ~/.bashmatic ]] || \
    bash -c "$(curl -fsSL https://bashmatic.re1.re); bashmatic-install -q -m https"
  return 0
}
----
