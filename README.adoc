...
image::doc/img/bashmatic-install.png[Installer,width=80%,align=left]
...
image::doc/img/docker-bash.png[Docker Build,width=80%,align=left]

Note how this dropped me straight into the Linux environment prompt with Bashmatic already installed.

=== Using Bashmatic In Your Scripts

There are four helper functions to assist with embedding Bashmatic into a new Shell script:

[source,bash]
----
❯ bashmatic.snippets.[TAB][TAB]
bashmatic.snippets.copy-for-paste     
bashmatic.snippets.print-for-install  
bashmatic.snippets.vscode             
bashmatic.snippets.vscode-overwrite
----

The last two are meant to be used with VSCode Snippets feature.

The first one can be used as is. You run it, and it will copy Bashmatic Initialization in the clipboard. Start a new file, a paste it. 

You will need to perform one global find/replace before this file is usable:

[source,regex]
:% s/\\\$/\$/g

Run the above in VIM to replace `\$` with `$` and the file should be ready to go.
...
# Install & Load Bashmatics
[[ -d "${HOME}/.bashmatic" ]] || \
  bash -c "$(curl -fsSL https://bashmatic.re1.re); bashmatic-install -q",
source ${BASHMATIC_HOME}/init.sh

# Start Using Bashmatic Helpers
...
image::doc/img/bashmatic-example.png[example,width=80%,align=left]
...
image::doc/img/k8installer.png[K8 Minicube Installer,width=80%,align=left]
...
image::doc/img/dev-setup.png[Developer Setup,width=80%,align=left]
...
image::doc/img/dbtop.png[DBTop Example,width=80%,align=left,link="https://github.com/kigster/bashmatic/blob/master/FUNCTIONS.adoc#db-top"]
...
image::doc/img/db.png[db usage,border=2,width=80%,align=left]
...
image::doc/img/db-commands.png[db usage,border=2,width=80%,align=left]

Altgernatively, here is the `--examples` view:

image::doc/img/db-examples.png[db examples,border=2,width=80%,align=left]
...
image::doc/img/db-connections.png[db usage,border=2,width=80%,align=left]
...
image::doc/img/db-pga.png[pg_activity,border=2,width=80%,align=left]
...
image::doc/img/bashmatic-tablet.png[Tablet Script in Action,border=2,width=80%,align=left]
...
image::doc/img/bashmatic-example.png[example,align=left,width=80%]
...
image::doc/img/specs.png[example,align=left,width=60%]
...
image::doc/img/specs-parallel.png[example,align=left,width=60%]
