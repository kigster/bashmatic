...
* The script will attempt to `git clone` the bashmatic repo into your home folder: `~/.bashmatic`, or (if it already exists), it will `git pull` latest changes.
...
* Once Brew is installed, it additionally ensures that you have `coreutils` and `gnu-sed` brew packages, both of which are required and are relied upon by Bashmatic.
* Finally, it will add the enabling hook to your `~/.bashrc` file.

Once Bashmatic is downloaded, you can run `bootstrap` script with `-h` to see this usage screen:
...
FLAGS:
  -b, --bash-version VERSION    Install VERSION instead of 5.1-rc2
