...
=== Example (E): `db` Shortcut for Database Utilities & `db top`
...
You can get a list of all availabled db connections with `db --connections` command:
...
====  Run Tests Sequentially `bin/specs` script

We use Bats framework for testing, however the convenient `bin/specs` script installs Bats and its dependencies so that we don't have to worry about installing it.
...
# run only test/db_test.bats, test/time_test.bats and test/utils_test.bats
bin/specs db time utils
----
