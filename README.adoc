...
After you install the library (the default location is `~/.bashmatic`), you have a choice of either sourcing the library in right from the start (by adding `source ~/.bashmatic/init.sh` do your shell initialization files, for example), or only loading it in the scripts that need that. Both approaches are valid and have their merits.

All we'll say on this matter is that we manage the optimize the hell out of the sourcing the library. Here is an example:

image::doc/img/bashmatic-init.png["source init.sh",80%,80%,align="center",border="5px"]
