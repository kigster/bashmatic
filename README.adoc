...
Sometimes you may not be able to use git (I've seen local certificate issues, and others), but maybe able to downlload with curl. In that case, you can lookup the latest release version number, and then issue this command:

[source,bash]
----
cd ${HOME}
curl --insecure -fSsl https://codeload.github.com/kigster/bashmatic/tar.gz/v1.6.0 -o bashmatic.tar.gz
rm -rf .bashmatic && tar xvzf bashmatic.tar.gz && mv bashmatic-* .bashmatic
