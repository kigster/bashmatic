...
* Finally, unless you specify `-l` or `--skip-on-login` the script will check your bash dot files, and will add the hook to load Bashmatic from either `~/.bashrc` or `${HOME}/.bash_profile`.
...
When you install Bashmatic it automatically adds a hook to your `${HOME}/.bash_profile`, but if you are on ZSH you may need to add it manually (for now).
