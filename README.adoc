...
Start exploring _Bashmatic®_ below with our <<usage-examples-for-the-impatient,examples section>>. When you are ready, the complete entire set of pubic functions (nearly 500 of those) can be found in the xref:FUNCTIONS.adoc[functions index page].

*For quick installation instructions, please <<installing-bashmatic,see the corresponding section>>*.
...
This final example is not just an example -- it's a working functioning script that can be used to install a bunch of cool stuff on your laptop. It's located in https://github.com/kigster/bashmatic/blob/master/bin/dev-setup[`bin/dev-setup`] and has many CLI flags:
