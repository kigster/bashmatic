...
After you install the library (the default location is `~/.bashmatic`), realize that you have a choice of either **sourcing the library from your shell 'dotfiles' like `~/.bash_profile` by adding `source ~/.bashmatic/init.sh`. OR can skip it during login initialization, and **only loading it in the scripts that use the library**.

[ATTENTION] Both approaches are absolutely valid and have their pros and cons. Loading bashmatic in your dotfiles could be a bit risky. One way or another we'll soon provide ways to verify that bashmatic you download is the safe and correct version, every time.


All we'll say on this matter is that we manage the optimize the hell out of the sourcing the library. Here is an example:

image::doc/img/bashmatic-init.png["source init.sh",80%,80%,align="center",border="5px",url="doc/img"]
