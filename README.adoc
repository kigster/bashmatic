...
NOTE: when we produced the screenshot we used `-q` flag to shorten the amount of screen output, otherwise it wouldnt' have fit onto one giant vertical monitor. But we don't recommend using `-q` when running `dev-setup` as it hides what's going on.
