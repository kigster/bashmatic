...
_Bashmatic®_'s programming style is heavily influenced by Ruby's DSL languages. If you take a quick look at the xref:lib/is.sh[is.sh] script, it defines a bunch of DSL functions that can be chained with `&&`  and `||` to create a compact and self-documenting code like this:
...
* We recently began providing function documentation using a fork of `shdoc` utility. You can find the auto-generated documentation in the xref:doc/USAGE.adoc[USAGE.adoc] file, or it's PDF render xref:doc/USAGE.pdf[USAGE.pdf].
...
* Additionally please checkout the xref:doc/CHANGELOG.adoc[CHANGELOG] and the xref:doc/LICENSE.adoc[LICENSE].
====
