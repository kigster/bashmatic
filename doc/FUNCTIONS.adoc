...
{ 
    ...
    git.repo-is-clean || { 
        output.is-ssh || { 
            output.is-terminal && bashmatic.is-developer && { 
                if [[ -f ${__bashmatic_auto_update_help_file} ]]; then
                    cat ${__bashmatic_auto_update_help_file}
                else
                    box.black-on-yellow "Bashmatic — I detected locally modified changes..." "Auto-update is therefore disabled until its git state is clean." | tee -a ${__bashmatic_auto_update_help_file} 1>&2
                fi
            }
        }
        return 1
...
success () 
{ 
    printf -- "\n${LibOutput__LeftPrefix}${txtblk}${bakgrn}  « SUCCESS »  ${clr} ${bldwht} ✔  ${bldgrn}$*${clr}" 1>&2
    printf -- "\n\n" 1>&2
}
