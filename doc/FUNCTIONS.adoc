...
{ 
    ...
    if [[ -f ${__bashmatic_auto_update_help_file} ]]; then
        ...
        box.black-on-yellow "${bldwht}Bashmatic Warning" "Local modifications have been detected in BASHMATIC_HOME=${BASHMATIC_HOME} " "Auto-update is therefore disabled until its git state is clean." | tee -a "${__bashmatic_auto_update_help_file}" 1>&2
    fi
}
...
{ 
    local dbname="$1"
    shift
    db.psql.connect "${dbname}" $(db.psql.args-data-only) -c '\\du' | sedx 's/\|.*$//g'
}
