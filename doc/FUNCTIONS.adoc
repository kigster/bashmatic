...
{ 
    ...
    local addup__cols=$@
    is-dbg && printf "addup__cols argument: ${addup__cols[*]}\n"
    for col in ${addup__cols[@]}
    do
        [[ $col =~ ^[0-9]+$ ]] || { 
    ...
    awk -v columns="${addup__cols[*]}" -v denominator="${addup__denominator}" -v multiplier="${addup__multiplier}" -v units="${addup__units}" -v debug="${addup__debug}" -f ~/.pu_addup.awk 0<&0
}
...
{ 
    ...
    ${bldylw}DESCRIPTION${clr}
      This is a streamlined interface to the UNIX  utility ${bldgrn}find${clr}.

    ${bldylw}USAGE
      ${bldgrn}files.search '*.mp3' [ ${txtpur}global flags to find${bldgrn} ] [ ${txtcyn}end flags to find ${bldgrn}]
      ${bldgrn}files.search '*.mp3' ${txtpur}'-maxdepth 1' ${txtcyn}'-exec cp -rp {} ~/Music/'${clr}
...
{ 
    ...
    local pattern="$1"
    shift
    local flags_before="$1"
    shift
    local flags_after="$1"
    shift
    [[ -z "${pattern}" ]] && return
    local cmd="find . ${flags_before} -type f -iname '${pattern}' ${flags_after} -print0"
    h1bg "${bldbl}${cmd}" 1>&2
    local stderr=$(file.temp)
    while IFS= read -d '' -r file; do
        echo "${file}"
    done < <(eval "${cmd}" 2>"${stderr}")
    [[ -s ${stderr} ]] && { 
        error "$(cat "${stderr}")"
        return 1
    }
...
{ 
    ...
    shift
    if [[ "$*" =~ "--" ]]; then
        error "Whoops!${txtrst} Did you specify arguments to convert via --option? \n"
    ...
    for f in $(find . -type f -name "*.${from_ext}" -depth 1 -print)
    do
        ...
        result=$?
        if [[ $result != 0 ]]; then
            error "convert exited with status $result, aborting...${txtrst}\n"
            return
        fi
        fsize=$(pu.fsize $f)
...
{ 
    ...
    local newname
    local -a file_array
    readarray -t file_array <<< "$(files.search "${pattern}")"
    h2bg "✅  ${bldblk} Discovered ${#file_array[@]} files matching ${bldblu}/${pattern}/"
    if is.a-function "${match}"; then
        for file in "${file_array[@]}"
        do
            eval rename.mv "${file}" "$(${match} "${file}")"
        done
    else
        if [[ -n "${match}" && -n "${replace}" ]]; then
            for file in "${file_array[@]}"
            do
                newname=$(echo "${file}" | sed -E "s#${match}#${replace}#${flags}")
                rename.mv "${file}" "${newname}"
            done
        else
            error "Invalid usage.${clr}
...
rename.do () 
{ 
    is-dbg && rename.print "${1}" "${2}"
    is-dbg || rename.mv "${1}" "${2}"
}
...
{ 
    local from="$1"
    local to="$2"
    error "An error occurred while attempting to move files:" "${bldylw}${from} ———→ ${bldpur}${to}"
    return 1
}
...
rename.kit.add-spec () 
{ 
    rename "$1" "__rename.kit.add-spec.func"
}
...
rename.kit.add-test () 
{ 
    rename "$1" "__rename.kit.add-test.func"
}
...
{ 
    ...
    local to="$2"
    [[ ! -f "${from}" ]] && { 
        error "Source file ${from} does not exists.\n"
        return 1
    }
    [[ -f "${to}" ]] && { 
        error "Destination file ${to} already exists.\n"
        return 2
    }
    [[ -z "${to}" ]] && { 
        error "Destination file appears to be blank.\n"
        return 3
    }
...
{ 
    ...
    is.a-non-empty-file ${file} || { 
        error "File ${file} is misssing"
        return 1
    }
    is.a-command qlmanage || { 
        error "Can't find qlmanage"
        return 2
    }
...
{ 
    ...
    unset user_input
    while true; do
        info "Your local Bashmatic is out of date — local version is ${bldylw}$(bashmatic.version)" 1>&2
        info "one of the following options depending on your security posture." 1>&2
        run.ui.ask-user-value user_input "$(txt-info)Auto-update Bashmatic? ${bldylw}(a,always | y,yes | n,no | x,never)" 1>&2
    ...
    if command -v open > /dev/null; then
        ...
        if command -v xdg-open > /dev/null; then
            ...
            if [[ $(uname -s) =~ Linux ]]; then
                error "To auto-open URLs on Linux, please install xdg-utils or flatpak-xdg-utils packages." 1>&2
            fi
        fi
    fi
    return 1
...
{ 
    ...
    h2 "Starting ffmpeg conversion from ${file} to ${output}, with algorithm #${ratio}" "Source file size is $(file.size.mb ${file})"
    run.set-next show-output-on
