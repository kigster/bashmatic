...
* [top-most-program()](#top-most-program)
...
walks the process tree up the chain until it finds the top
...
: A generic menu select from a list of values.
...
* [brew.bundle.document()](#brewbundledocument)

### `package.is-installed()`

For each passed argument checks if it's installed.

### `brew.bundle.document()`

Add descriptions to all packages and casks listed int the Brewfile
...
* [git.cfgu()](#gitcfgu)
* [git.repo()](#gitrepo)
* [git.repo.readme-url()](#gitreporeadme-url)
...
* [7z.archive()](#7zarchive)

### `7z.archive()`

This function receives as the first argument the name of the
...
* [net.host.ips()](#nethostips)
* [net.hosts.ips()](#nethostsips)
...
Resolves the IP address of a host and returns a single IP. If 
the host has multiple IP addresses, it returns the last one, sorted numerically.

### `net.host.ips()`

Resolves the IP addresses of a host and returns them as a space-separated list suitable
for insertion into a shell array.

### `net.hosts.ips()`

Resolves the IP addresses of a list of hosts and returns them as a space-separated list suitable
for insertion into a shell array.
...
* [ruby.fix-git-smart()](#rubyfix-git-smart)

### `ruby.fix-git-smart()`

Patch git smart to remove File.exists?() and replace with File.exist?()
...
## File `bin/install-ruby-interactive`
 ...
 * Copyright © 2017-2026 Konstantin Gredeskoul, All rights reserved.
